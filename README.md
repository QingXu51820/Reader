...
## 目录解析说明
1. 默认目录解析支持识别「`-数字-` + 下一行标题」结构（例如 `-1-`、`— 1 —`、`【1】`、`(1)`、`第1`），会在 1-3 行内跳过空行或分隔行，合并为目录标题：`<数字> <标题>`。  
2. 自定义正则目录解析优先级最高，启用后将覆盖默认解析逻辑。  

## 显示设置：主题模式
1. 在【设置】>【显示设置】新增“主题模式（明亮/暗黑）”切换，默认明亮模式。  
2. 切换主题会写入一组推荐的背景/正文/标题颜色；如果之后在显示设置里手动改色，将覆盖主题预设颜色。  
